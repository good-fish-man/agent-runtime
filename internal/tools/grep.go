...
func (t *GrepTool) InvokableRun(ctx context.Context, input string, opts ...tool.Option) (string, error) {
	...
	if _, err := exec.LookPath("rg"); err != nil {
		matches, err := grepWithoutRipgrep(ctx, grepInput, basePath)
		if err != nil {
			return "", err
		}
		result := GrepOutput{
...
func grepWithoutRipgrep(ctx context.Context, in GrepInput, basePath string) ([]GrepMatch, error) {
	matcher, err := newLineMatcher(in.Pattern, in.Regex, in.CaseSensitive, in.Invert)
	if err != nil {
		return nil, err
	}
	...
	visit := func(path string) error {
		if in.Glob != "" {
			...
			if !ok {
				ok, _ = filepath.Match(in.Glob, filepath.ToSlash(path))
			}
			if !ok {
				return nil
			}
		}
		fileMatches, err := scanGrepFile(ctx, path, matcher, in.Head-len(matches))
		if err != nil {
			return nil
		}
	...
	_ = filepath.WalkDir(basePath, func(path string, d fs.DirEntry, err error) error {
		...
		if in.Head > 0 && len(matches) >= in.Head {
			return fs.SkipAll
		}
		return visit(path)
	})
...
func newLineMatcher(pattern string, regex, caseSensitive, invert bool) (func(string) bool, error) {
	needle := pattern
	if !caseSensitive {
...
func scanGrepFile(ctx context.Context, path string, matcher func(string) bool, remaining int) ([]GrepMatch, error) {
	if remaining == 0 {
		return nil, nil
	}
	...
	for scanner.Scan() {
		...
		line := scanner.Text()
		if matcher(line) {
			matches = append(matches, GrepMatch{File: path, Line: lineNo, Text: line})
