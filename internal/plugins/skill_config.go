...
func DefaultSkillConfigPath() string {
	// 优先使用 SKILLS_CONFIG_PATH 环境变量
	if cfgPath := os.Getenv(constant.EnvSkillsConfigPath); cfgPath != "" {
		if _, err := os.Stat(cfgPath); err == nil {
			return cfgPath
		}
	}

	// 默认使用 <baseDir>/config/skills-config.yaml
	configPath := filepath.Join(GetBaseDir(), constant.SkillsConfigRelPath)
