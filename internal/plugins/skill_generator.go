...
func NewSkillGenerator() *SkillGenerator {
	return NewSkillGeneratorWithDir(GetSkillsDir())
}

// NewSkillGeneratorWithDir creates a generator targeting the effective skills directory.
func NewSkillGeneratorWithDir(skillsDir string) *SkillGenerator {
...
func NewCreateSkillTool() *CreateSkillTool {
	return NewCreateSkillToolForDir(GetSkillsDir())
}

// NewCreateSkillToolForDir keeps newly created skills in the same directory
// used for discovery, so they become available on the next request.
func NewCreateSkillToolForDir(skillsDir string) *CreateSkillTool {
	return &CreateSkillTool{
		generator: NewSkillGeneratorWithDir(skillsDir),
	}
}
...
func (t *CreateSkillTool) InvokableRun(ctx context.Context, argumentsInJSON string, opt ...tool.Option) (string, error) {
	...
	// 确定存储路径
	skillsDir := t.generator.skillsDir
	if input.Category != "" {
