...
func (t *CollectTaskTool) InvokableRun(ctx context.Context, argumentsInJSON string, opt ...tool.Option) (string, error) {
	...
	if timeout > constant.MaxSubAgentWaitTimeoutSec {
		timeout = constant.MaxSubAgentWaitTimeoutSec // 最大 5 分钟
	}
...
func (t *ParallelSpawnTool) InvokableRun(ctx context.Context, argumentsInJSON string, opt ...tool.Option) (string, error) {
	...
	if timeout == 0 {
		timeout = constant.DefaultParallelTaskTimeoutSec * time.Second
	}
