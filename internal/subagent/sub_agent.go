...
import (
	...
	runtimeeino "github.com/good-fish-man/agent-runtime/internal/eino"
)
...
func newSubAgent(ctx context.Context, cfg *SubAgentConfig, defaultModel model.ToolCallingChatModel, agentTools []tool.BaseTool) (*SubAgent, error) {
	// 构建 instruction
	...
	if maxIterations > constant.DefaultMaxIterations {
		maxIterations = constant.DefaultMaxIterations
	}
...
func (s *SubAgent) Run(ctx context.Context, task string) error {
	...
	maxTimeoutMs := constant.MaxSubAgentWaitTimeoutSec * 1000
