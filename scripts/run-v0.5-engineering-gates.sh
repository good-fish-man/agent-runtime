...
CACHE_ROOT="${ATHENA_GATE_CACHE_ROOT:-${TMPDIR:-/tmp}/athena-v0.5-gates}"
...
run "deployment protocol contract and validation" \
  env GOCACHE="$CACHE_ROOT/protocol" GOWORK=off go -C "$WORKSPACE_ROOT/athena-protocol" test ./protocol/deployment/v1
...
run "deployment lifecycle, manifest, shadow, canary, rollback, scope, and migration gates" \
  env GOCACHE="$CACHE_ROOT/client" go -C "$WORKSPACE_ROOT/agent-runtime-client" test \
    ./application/service/deployment ./application/service/runtime ./infra/repository/migration ./infra/repository/repo/deployment
...
printf '\n[v0.5] ENGINEERING_VERIFIED\n'
printf '[v0.5] This result does not close the external v0.3 release-evidence gates.\n'
